<#
.SYNOPSIS
  The "Get-ADObjectTimestamp" function takes a numeric Active Directory timestamp and converts it to a human-readable format (as a DateTime object).
...
.EXAMPLE
  ...
  Name              : Jannus Fugal
  ObjectClass       : user
  ObjectGUID        : ffa5f6e4-dfbe-4396-8496-3f44a857f065
  SamAccountName    : Jannus.Fugal
  SID               : S-1-5-21-102932503-109117628-3773961456-57276
  Surname           : Fugal
  ...
  Here we query Active Directory for the "badPasswordTime" property of a specific user.  We then call the function "Convert-DateTimeFromADObjectTimestamp" by using its built-in alias of 'ConvertWindowsTimestamp'.

.INPUTS
.OUTPUTS
.NOTES
  Name: Convert-DateTimeFromADObjectTimestamp.ps1
  Author: Travis Logue
  ...
  .
#>
function Convert-DateTimeFromADObjectTimestamp {
  [CmdletBinding()]
